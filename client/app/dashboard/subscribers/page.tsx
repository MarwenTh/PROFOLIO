...
export default function SubscribersPage() {
  ...
  const {
    ...
    removeSubscriber,
    importSubscribers,
  } = useSubscribers(selectedPortfolioId);
  const { newsletters, loading: newslettersLoading } =
    useNewsletters(selectedPortfolioId);
  ...
  const emailsSent = newsletters
    .filter((n) => n.status === "sent")
    .reduce((acc, n) => acc + n.sent_to, 0);
  ...
  return (
    <div className="space-y-10">
      <PageHeader
        title="Subscribers"
        description="Manage your newsletter subscribers and email campaigns."
      />
        ...
        {stats.map((stat) => (
          <DashboardCard key={stat.label} padding="small">
            <div className="flex items-center justify-between mb-4">
              <div
                className={`w-12 h-12 rounded-2xl ${colorMap[stat.color]} flex items-center justify-center`}
              >
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-neutral-200 dark:border-neutral-800">
                  <th className="text-left py-4 px-4 text-xs font-black uppercase tracking-widest text-neutral-400">
                    Email
                  </th>
                  <th className="text-left py-4 px-4 text-xs font-black uppercase tracking-widest text-neutral-400">
                    Name
                  ...
                  <th className="text-right py-4 px-4 text-xs font-black uppercase tracking-widest text-neutral-400">
                    Actions
                  </th>
                </tr>
              </thead>
