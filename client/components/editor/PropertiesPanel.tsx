...
export const PropertiesPanel = ({
  ...
  const isSection = ["header", "body", "footer", "custom"].includes(
    component.type,
  );
  ...
  return (
    <div className="w-[320px] bg-[#0F0F10] border-l border-white/5 flex flex-col h-full overflow-hidden text-neutral-300 font-sans shadow-2xl">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-6">
        ...
        {/* Generic Block Content Editor */}
        {component.type.startsWith("block-") &&
          typeof component.content === "object" && (
            <div className="space-y-4">
              <SectionHeader icon={Type} label="Block Content" />
              {Object.entries(component.content).map(([key, value]) => {
                // Skip complex objects for now, only edit primitives
                if (
                  typeof value === "object" &&
                  value !== null &&
                  !Array.isArray(value)
                  ...
                  return (
                    <div key={key} className="space-y-1">
                      ...
                      <input
                        ...
                        className="w-full bg-[#1A1A1E] border border-white/5 rounded-lg p-2 text-xs text-white outline-none focus:border-indigo-500/50"
                        placeholder="https://..."
                      />
                    </div>
                  );
                ...
                // Default text input (or textarea for longer text)
                const isLongText =
                  ...
                  <div key={key} className="space-y-1">
                    ...
                    {isLongText ? (
                      <textarea
                        ...
                        className="w-full bg-[#1A1A1E] border border-white/5 rounded-lg p-2 text-xs text-white outline-none focus:border-indigo-500/50 resize-y min-h-[60px]"
                      />
                    ) : (
                      <input
                        type="text"
        ...
        {["container", "row", "column"].includes(component.type) && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              <IconButton
                active={
                  component.styles.flexDirection === "row" ||
                  (!component.styles.flexDirection && component.type === "row")
                }
            ...
            <div className="space-y-2">
              ...
              <div className="flex bg-[#1A1A1E] rounded-lg p-1 border border-white/5">
                <IconButtonSmall
                  icon={AlignLeft}
                  active={component.styles.justifyContent === "flex-start"}
                  onClick={() => updateStyle("justifyContent", "flex-start")}
                />
                <IconButtonSmall
                  icon={AlignCenter}
                  active={component.styles.justifyContent === "center"}
                  onClick={() => updateStyle("justifyContent", "center")}
                />
                <IconButtonSmall
                  icon={AlignRight}
                  active={component.styles.justifyContent === "flex-end"}
                  onClick={() => updateStyle("justifyContent", "flex-end")}
                />
                <IconButtonSmall
                  icon={AlignJustify}
                  active={component.styles.justifyContent === "space-between"}
            ...
            <div className="space-y-2">
              ...
              <div className="flex bg-[#1A1A1E] rounded-lg p-1 border border-white/5">
                <IconButtonSmall
                  ...
                  active={component.styles.alignItems === "flex-start"}
                  onClick={() => updateStyle("alignItems", "flex-start")}
                />
                <IconButtonSmall
                  icon={AlignCenter}
                  active={component.styles.alignItems === "center"}
                  onClick={() => updateStyle("alignItems", "center")}
                />
                <IconButtonSmall
                  icon={StretchHorizontal}
                  active={component.styles.alignItems === "flex-end"}
                  onClick={() => updateStyle("alignItems", "flex-end")}
                />
                <IconButtonSmall
                  icon={StretchVertical}
                  active={component.styles.alignItems === "stretch"}
        ...
        {(component.type === "text" || component.type === "button") && (
          <div className="space-y-4">
            ...
            <div className="flex bg-[#1A1A1E] rounded-lg p-1 border border-white/5 w-fit">
              <IconButtonSmall
                icon={AlignLeft}
                active={component.styles.textAlign === "left"}
                onClick={() => updateStyle("textAlign", "left")}
              />
              <IconButtonSmall
                icon={AlignCenter}
                active={component.styles.textAlign === "center"}
                onClick={() => updateStyle("textAlign", "center")}
              />
              <IconButtonSmall
                icon={AlignRight}
                active={component.styles.textAlign === "right"}
          ...
          {isSection ? (
            <div className="space-y-4">
              <NumberInput
                ...
                onChange={(v: number) =>
                  onUpdate(component.id, { height: v } as any)
                }
                suffix="px"
              />
            </div>
            ...
            <>
              <div className="grid grid-cols-2 gap-3">
                <NumberInput
                  ...
                  onChange={(v: number) =>
                    onUpdate(component.id, { x: v } as any)
                  }
                  suffix="px"
                />
                <NumberInput
                  ...
                  onChange={(v: number) =>
                    onUpdate(component.id, { y: v } as any)
                  }
                  suffix="px"
                />
              </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <option value="0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.04)">
                Large
              </option>
              <option value="0 25px 50px -12px rgba(0, 0, 0, 0.25)">
                Extra Large
              </option>
              <option value="0 0 15px rgba(255, 255, 255, 0.1)">
                Glow (White)
              </option>
              <option value="0 0 15px rgba(99, 102, 241, 0.3)">
                Glow (Indigo)
              </option>
            </select>
          </div>
            ...
            {component.animation && (
              <>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <label
                    htmlFor="anim-once"
                    className="text-[10px] text-neutral-400 font-bold uppercase tracking-wider cursor-pointer"
                  >
                    Animate Once
                  </label>
                </div>
...
const NumberInput = ({ label, value, onChange, suffix, step }: any) => (
  <div className="space-y-1">
    ...
    <div className="relative">
      ...
      />
      <span className="absolute right-2 top-1/2 -translate-y-1/2 text-[10px] text-neutral-600 font-bold">
        {suffix}
      </span>
    </div>
  </div>
