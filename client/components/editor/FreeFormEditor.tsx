...
const EditorLayout = () => {
    ...
    const selectedComponent = sections.flatMap(s => s.components).find(c => c.id === selectedId);
                ...
                {(selectedComponent || selectedSection) && (
                    <div className="absolute top-0 right-0 w-[320px] h-full border-l border-white/5 z-40 bg-[#0F0F10] overflow-y-auto custom-scrollbar shadow-2xl">
                        <PropertiesPanel 
                            ...
                            onDelete={(selectedComponent ? removeComponent : removeSection) as any}
                        />
                    </div>
                )}
            </div>
...
export const FreeFormEditor = ({ initialSections = [] }: { initialSections?: any[] }) => {
    return (
