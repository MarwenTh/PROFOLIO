...
export function useMarketplace(filters?: { type?: string; search?: string; minPrice?: number; maxPrice?: number }) {
  ...
  const fetchItems = async () => {
    try {
      ...
      toast.error(error.response?.data?.message || 'Failed to fetch marketplace items');
    } finally {
      setLoading(false);
    }
  };
...
export function useMyCreations() {
  ...
  const fetchCreations = async () => {
    try {
      ...
      toast.error(error.response?.data?.message || 'Failed to fetch your creations');
    } finally {
      setLoading(false);
    }
  };
  ...
  return { items, loading, createItem, updateItem, deleteItem, refetch: fetchCreations };
}
...
export function usePurchases() {
  ...
  return { purchases, loading, purchaseItem, refetch: fetchPurchases };
}
...
export function useSavedItems() {
  ...
  const fetchSavedItems = async () => {
    try {
      ...
      toast.error(error.response?.data?.message || 'Failed to fetch saved items');
    } finally {
      setLoading(false);
    }
  };
